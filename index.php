...
// Global JavaBridge Configuration for OmanNet (Standalone Mode)
// This must be at the VERY START to prevent shadowing by other inclusions
if (!defined("JAVA_HOSTS")) {
    define("JAVA_HOSTS", "127.0.0.1:8085");
}
if (!defined("JAVA_SERVLET")) {
    define("JAVA_SERVLET", "Off");
}


// Start output buffering to catch any accidental output (prevents HTML in JSON responses)
...
// Don't display errors - log them instead (prevents HTML in JSON responses)
...
// Strip base path prefixes if present (for WAMP Apache)

// This allows routes to work both with and without the prefix

// Try to strip the full path first, then fall back to just /php-backend

$basePaths = [

    '/frontend/php-frontend/php-backend',
